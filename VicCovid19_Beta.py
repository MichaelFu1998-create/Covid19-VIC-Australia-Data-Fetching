...
def unix_time_millis(date):
    """
    change str formatted date to milliseconds since 1970
    *there is a special format for date -> 'yyyy-mm-dd'    
    e.g. 6th Aug 2020 -> '2020-08-06' 
    
    Parameters
    ----------
    date : str
        the date of data to be fetched
        format : yyyy-mm-dd e.g. 8th Jan 2019 -> '2019-01-08'
    
    Returns
    -------
    milliseconds since 1970 converted from date
    """ 

    date = date + ' 12:00:00 AM'
    date = dt.strptime(date, '%Y-%m-%d %I:%M:%S %p')
    epoch = dt.utcfromtimestamp(0)
    return int((date - epoch).total_seconds() * 1000.0)


def append_specific_daily_cases(date, excel_file_name = EXCEL_FILE_NAME):
    """
    append data on the specific date into designated excel file
    * there is a special format for the param date -> 'yyyy-mm-dd'    
    e.g. 6th Aug 2020 -> '2020-08-06' 
    ...
    * special format needed
    ...
    date_millisec = unix_time_millis(date)
    url = 'https://services1.arcgis.com/vHnIGBHHqDR6y0CR/arcgis/rest/services/Victorian_LGA_Cases/FeatureServer/0/query?where=1%3D1&outFields=*&outSR=4326&f=json&LastUpdated='
    url += str(date_millisec)
    r_json = fetch_data_from_URL_query(url)
    data = r_json['features']
...
def append_latest_daily_cases(excel_file_name = EXCEL_FILE_NAME):
    """
...
#create_excel_with_LGA_data()
#append_specific_daily_cases('2020-07-19')
date = '2020-07-19'
